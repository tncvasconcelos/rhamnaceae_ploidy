...
#load("netdiv_map.Rsave")
#load("distribution_results/species_richness_map.Rsave")
res_test <- GetResDistribution(raster1=sprichness_map, raster2=netdiv_map)

australia <- subset(wrld_simpl, NAME=="Australia")

pdf("maps_10thpercentile.pdf", height=3.5, width=10)
# plots
par(mfrow=c(1,3))
par(oma=c(1,1,1,1))
#par(mar = c(2, 5, 2, 5))
...
raster::plot(sprichness_map_aus, col=rev(pal), xlim=c(110,160), ylim=c(-47,-5))
title(main="species richness")
plot(australia, add=T)

# net_div
netdiv_map_aus <- mask(netdiv_map, australia)
raster::plot(netdiv_map_aus, col=rev(pal), xlim=c(110,160), ylim=c(-47,-5))
...
plot(res_test, breaks=breakpoints, col = colors, xlim=c(110,160), ylim=c(-47,-5))
...
species_richness_map_woPomaderris <- readRDS("species_richness_map_woPomaderris_10thpercentile.Rdata")
plot(species_richness_map_woPomaderris)

pdf("sprich_wo_Pomaderris_10thpercentile.pdf")
pal <- hcl.colors(30, palette = "RdYlBu", alpha = 1)
